<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="票据名称" prop="piaojumingcheng">
          <el-input v-model="ruleForm.piaojumingcheng" 
              placeholder="票据名称" clearable  :readonly="ro.piaojumingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="票据名称" prop="piaojumingcheng">
              <el-input v-model="ruleForm.piaojumingcheng" 
                placeholder="票据名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.piaojutupian" label="票据图片" prop="piaojutupian">
          <file-upload
          tip="点击上传票据图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.piaojutupian?ruleForm.piaojutupian:''"
          @change="piaojutupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.piaojutupian" label="票据图片" prop="piaojutupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.piaojutupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="商家名称" prop="shangjiamingcheng">
          <el-select :disabled="ro.shangjiamingcheng" @change="shangjiamingchengChange" v-model="ruleForm.shangjiamingcheng" placeholder="请选择商家名称">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.shangjiamingcheng" label="商家名称" prop="shangjiamingcheng">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="商店地址" prop="shangdiandizhi">
          <el-input v-model="ruleForm.shangdiandizhi" 
              placeholder="商店地址" clearable  :readonly="ro.shangdiandizhi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="商店地址" prop="shangdiandizhi">
              <el-input v-model="ruleForm.shangdiandizhi" 
                placeholder="商店地址" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="票据详情" prop="piaojuxiangqing">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.piaojuxiangqing" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.piaojuxiangqing" label="票据详情" prop="piaojuxiangqing">
                    <span v-html="ruleForm.piaojuxiangqing"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	youxiaoqi : false,
	zhuyishixiang : false,
	piaojuxiangqing : false,
	shangjiamingcheng : false,
	shangdiandizhi : false,
	clicktime : false,
	clicknum : false,
      },
      ruleForm: {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='piaojumingcheng'){
            this.ruleForm.piaojumingcheng = obj[o];
	    this.ro.piaojumingcheng = true;
            continue;
          }
          if(o=='piaojutupian'){
            this.ruleForm.piaojutupian = obj[o];
	    this.ro.piaojutupian = true;
            continue;
          }
          if(o=='fenlei'){
	    ...
	    this.ro.zhuyishixiang = true;
          ...
          if(o=='piaojuxiangqing'){
            this.ruleForm.piaojuxiangqing = obj[o];
	    this.ro.piaojuxiangqing = true;
            continue;
          }
	    ...
	    this.ro.shangjiamingcheng = true;
          ...
          if(o=='shangdiandizhi'){
            this.ruleForm.shangdiandizhi = obj[o];
	    this.ro.shangdiandizhi = true;
            continue;
          }
          if(o=='clicktime'){
    ...
    // 下二随
    shangjiamingchengChange () {
      this.$http({
        url: `follow/shangjiaxinxi/shangjiamingcheng?columnValue=`+ this.ruleForm.shangjiamingcheng,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.shangdiandizhi){
                this.ruleForm.shangdiandizhi = data.data.shangdiandizhi
              }
        } else {
          this.$message.error(data.msg);
        }
	...
	this.ruleForm.piaojuxiangqing = this.ruleForm.piaojuxiangqing.replace(reg,'../../../springboot4962b/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.piaojutupian!=null) {
		this.ruleForm.piaojutupian = this.ruleForm.piaojutupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhekoupiao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhekoupiaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    piaojutupianUploadChange(fileUrls) {
	this.ruleForm.piaojutupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
