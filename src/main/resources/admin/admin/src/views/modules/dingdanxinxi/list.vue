<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('dingdanxinxi','退票') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="tuipiaoxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'退票':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','退票') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="tuipiaoxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'退票':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','退票') && contents.tableBtnIcon == 0" type="success" size="mini" @click="tuipiaoxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'退票':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','出票') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="chupiaoxinxiCrossAddOrUpdateHandler(scope.row,'cross','','zhuangtai','该票已使用','已使用,未使用'.split(',')[0])">{{ contents.tableBtnFont == 1?'出票':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','出票') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="chupiaoxinxiCrossAddOrUpdateHandler(scope.row,'cross','','zhuangtai','该票已使用','已使用,未使用'.split(',')[0])">{{ contents.tableBtnFont == 1?'出票':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','出票') && contents.tableBtnIcon == 0" type="success" size="mini" @click="chupiaoxinxiCrossAddOrUpdateHandler(scope.row,'cross','','zhuangtai','该票已使用','已使用,未使用'.split(',')[0])">{{ contents.tableBtnFont == 1?'出票':'' }}</el-button>
    ...
    <chupiaoxinxi-cross-add-or-update v-if="chupiaoxinxiCrossAddOrUpdateFlag" :parent="this" ref="chupiaoxinxiCrossaddOrUpdate"></chupiaoxinxi-cross-add-or-update>
...
import chupiaoxinxiCrossAddOrUpdate from "../chupiaoxinxi/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      chupiaoxinxiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    tuipiaoxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.tuipiaoxinxiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
    ...
    chupiaoxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.chupiaoxinxiCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','dingdanxinxi');
      this.$storage.set('statusColumnName',statusColumnName);
      this.$storage.set('statusColumnValue',statusColumnValue);
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.chupiaoxinxiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.chupiaoxinxiCrossaddOrUpdate.init(row.id,type);
      });
    },
    payHandler(row){
    ...
    chartDialog() {
      ...
      this.$nextTick(()=>{
        ...
        // xcolumn ycolumn shangjiamingcheng jine
        var jineChart = this.$echarts.init(document.getElementById("jineChart"),'macarons');
        this.$http({
            url: `dingdanxinxi/value/shangjiamingcheng/jine`,
            method: "get"
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
    ...
    init () {
          this.zhuangtaiOptions = "已使用,未使用".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
